...
public class JwtUtil {
    ...
    // 개발자가 yaml 파일을 깃허브나 올라가면 안되는 공개된 공간에 올릴 경우를 대비해서 변환 처리
    private SecretKey key;
    // js async await 비슷한 효과를 가진 어노테이션
    @PostConstruct
    public void init(){
        //this.key = private SecretKey key;
        //            secretKeyValue 를 yaml에서 가져온 다음 데이터 변환을 하고 secretKey key 넣는다는 것이구나.
    ...
    // 만약에 매서드 내부 () 내부에 어떤 자료형을 넣어야하는지 모르겠다면
    ...
    public boolean isValidToken(String token) {
        try {
            ...
            log.debug("Token validation : {}", e.getMessage());
            return false;
        }
    }
