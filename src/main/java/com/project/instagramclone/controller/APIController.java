 ...
 * Model model
 * model.addAttribute("JSP에서 사용할 변수 명칭", JSP로 전달할 데이터);
 *
 * 로 전달하는 데이터를 백엔드에서 제대러 호출해서 전달하고 있는게 맞는지 확인하기 위한 용도
 * 백엔드 개발자는 viewController를 만들 생각을 하지 않고,
 * 보통은 APIController와 같은 컨트롤러만 만들고, 프로트엔드에게
 * api/endpoint를 전달한다.
 */
