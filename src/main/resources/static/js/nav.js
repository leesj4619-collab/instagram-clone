...
const 인기태그불러오기=async()=> {
    try {
        ...
        if (!백엔드응답결과.ok) throw new Error("해시태그백엔드에서가져오기 실패 :", 백엔드응답결과.status);
...
const 태그목록렌더링=(태그목록)=>{
    ...
    태그목록.forEach(function(태그){
        ...
        항목.innerHTML =
            ...
            + '<span class="hashtag-count">' + 태그.count + '새 게시물</span>>';
